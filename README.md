...
$$
\frac{\mathrm{EMA}p(t) - \mathrm{EMA}{p+1}(t)}
{|\mathrm{EMA}_{p+1}(t)|}
\times \ln(p + 1)
$$
...
\sum_{p=\mathrm{EMA}{\min}}^{\mathrm{EMA}{\max}-1}
\left[
\frac{\mathrm{EMA}p(t) - \mathrm{EMA}{p+1}(t)}
{|\mathrm{EMA}_{p+1}(t)|}
\times \ln(p + 1)
...
$$
\mathrm{EMA}p(t) > \mathrm{EMA}{p+1}(t)
$$
...
$$
\mathrm{volume_score}(t)

\sum_{p=150}^{199}
\left[
\frac{\mathrm{VolumeEMA}p(t) - \mathrm{VolumeEMA}{p+1}(t)}
...
\left(
\frac{\mathrm{price}(t)}
{\mathrm{price}(t-\mathrm{window})}
\right)^{\frac{1}{\mathrm{window}}}
...
$$
\mathrm{current_effort_per_1pct}(t)

\frac{\mathrm{volume}(t)}
{\mathrm{abs_move_pct}(t)}
$$
...
$$
\mathrm{past_avg_effort_per_1pct}(t)

\frac{
\sum_{i=t-\mathrm{window}}^{t-1} \mathrm{volume}(i)
}{
\sum_{i=t-\mathrm{window}}^{t-1} \mathrm{abs_move_pct}(i)
}
$$
...
$$
\mathrm{past\ effort\ per\ 1%}

\mathrm{current\ effort\ per\ 1%}
$$
...
$$
\mathrm{low_threshold}
\le
\mathrm{price_score}(t)
\le
\mathrm{high_threshold}
$$
